...
def get_pcd(scene_name, color_name, rgb_path, mask_generator, save_2dmask_path, sem_mask_generator=None):
    ...
    #semantic_map = cv2.resize(color_image, (640, 480))
    ...
    if sem_mask_generator is not None:
        sem_map = sem_mask_generator(color_image)
        sem_map = group_sem_voting(sem_map, group_ids, group_ids.max()+1)
    ...
    save_dict = dict(coord=points_world[:,:3], color=colors, group=group_ids, sem_map=sem_map)
    return save_dict
...
def cal_group(input_dict, new_input_dict, match_inds, ratio=0.5):
    ...
    sem_0 = input_dict['sem_map']
    sem_1 = new_input_dict["sem_map"]
    ...
    for group_i, overlap_count in group_overlap.items():
        ...
        if count / total_count >= ratio:
            if group_1_counts[group_i] < group_0_counts[group_j]:
                sem_1[group_1 == group_i] = sem_0[group_0 == group_j][0]
            group_1[group_1 == group_i] = group_j
            
    return group_1, sem_1
...
def cal_2_scenes(pcd_list, index, voxel_size, voxelize, th=50):
    ...
    pcd1_new_group, pcd1_new_sem = cal_group(input_dict_0, input_dict_1, match_inds)
    ...
    input_dict_1["sem_map"] = pcd1_new_sem
    pcd0_new_group, pcd0_new_sem = cal_group(input_dict_1, input_dict_0, match_inds)
    ...
    pcd_new_sem = np.concatenate((pcd0_new_sem, pcd1_new_sem), axis=0)
    pcd_dict = dict(coord=pcd_new_coord, color=pcd_new_color, group=pcd_new_group, sem_map=pcd_new_sem)
